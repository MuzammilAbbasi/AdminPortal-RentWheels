...
export function fetchSelfRegistration(
  ...
  const baseURL1 = `${baseURL}${config.endpoint.backoffice_ib.selfRegistration.selfRegistrationIndex}`;
  const URL = baseURL1
...
export function fetchSelfRegistrationExportExcel(
  onSuccess,
  onFailure
){
  const baseURL = config.url.ib_backoffice
  console.log("baseURL1 ==>",baseURL)
  const baseURL1 = `${baseURL}${config.endpoint.backoffice_ib.selfRegistration.selfRegistrationUserExportExcel}`;
  const URL = baseURL1
  console.log("fetchIps url: ", URL);
  console.log(getHeaders()," <== HEADERS")
